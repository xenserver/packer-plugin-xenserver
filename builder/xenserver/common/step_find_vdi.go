...
func (s *StepFindVdi) Run(state multistep.StateBag) multistep.StepAction {
	...
	vdis, err := client.GetVdiByNameLabel(s.VdiName)

	switch {
	case len(vdis) == 0:
		ui.Error(fmt.Sprintf("Couldn't find a VDI named '%s'", s.VdiName))
		return multistep.ActionHalt
	case len(vdis) > 1:
		ui.Error(fmt.Sprintf("Found more than one VDI with name '%s'. Name must be unique", s.VdiName))
		return multistep.ActionHalt
	}
	...
	if err != nil {
		ui.Error(fmt.Sprintf("Unable to get UUID of VDI '%s': %s", s.VdiName, err.Error()))
		return multistep.ActionHalt
	}
